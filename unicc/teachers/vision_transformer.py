...
class DinoVisionTransformer(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        print('inchans ahahahah', self.in_chans)
        ...
        if ffn_layer == "mlp":
            print("DINO VisionTransformer - using MLP layer as FFN")
            ffn_layer = Mlp
        elif ffn_layer == "swiglufused" or ffn_layer == "swiglu":
            print("DINO VisionTransformer - using SwiGLU layer as FFN")
            ffn_layer = SwiGLUFFNFused
        elif ffn_layer == "identity":
            print("DINO VisionTransformer - using Identity layer as FFN")
    ...
    def prepare_tokens_with_masks(self, x, masks=None, registers=None):
        ...
        if self.register_tokens is not None:
            assert (
                registers is None
            ), "either self.register_tokens or registers can be used, not both"
            _register_tokens = self.register_tokens.expand(x.shape[0], -1, -1)
        elif registers is not None:
            _register_tokens = registers.expand(x.shape[0], -1, -1)

        _num_reg_tokens = 0
        if _register_tokens is not None:
            x = torch.cat((x[:, :1], _register_tokens, x[:, 1:]), dim=1)
            _num_reg_tokens = _register_tokens.shape[1]
    ...
    def forward_features(self, x, masks=None, registers=None):
        ...
        return {
            ...
            "x_norm_regtokens": x_norm[:, 1 : num_reg_tokens + 1],
            "x_norm_patchtokens": x_norm[:, num_reg_tokens + 1 :],
            "x_prenorm": x,
            "masks": masks,
        }

    def _get_intermediate_layers_not_chunked(self, x, n=1, registers=None):
        x, num_registers = self.prepare_tokens_with_masks(x, registers=registers)
        ...
        return output, num_registers

    def _get_intermediate_layers_chunked(self, x, n=1, registers=None):
        x, num_registers = self.prepare_tokens_with_masks(x, registers=registers)
    ...
    def get_intermediate_layers(
        ...
        if self.chunked_blocks:
            outputs, num_registers = self._get_intermediate_layers_chunked(
                x, n, registers=registers
            )
        else:
            outputs, num_registers = self._get_intermediate_layers_not_chunked(
                x, n, registers=registers
            )
        ...
        # Splitting the output into class, register and patch tokens
        class_tokens = [out[:, 0] for out in outputs]
        register_tokens = [out[:, 1 : num_registers + 1] for out in outputs]
        patch_tokens = [out[:, num_registers + 1 :] for out in outputs]
        ...
        if return_class_token and return_register_tokens:
            return tuple(zip(patch_tokens, class_tokens, register_tokens))
        elif return_class_token:
            return tuple(zip(patch_tokens, class_tokens))
        elif return_register_tokens:
            return tuple(zip(patch_tokens, register_tokens))
        else:
...
def vit_tiny(patch_size=16, num_register_tokens=0, **kwargs):
    print('jkasjks',kwargs.get("in_chans", 3))
